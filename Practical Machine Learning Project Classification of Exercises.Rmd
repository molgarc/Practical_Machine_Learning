...
In this section, the cross validation process and the analysis of the multilass classification algorithms are performed. 
...
Other methods for cross validation could have been selected. The available resampling methods are: The  "boot", "boot632", "optimism_boot", "boot_all", "cv", "repeatedcv", "LOOCV", "LGOCV" (for repeated training/test splits), "none" (only fits one model to the entire training set), "oob" (only for random forest, bagged trees, bagged earth, bagged flexible discriminant analysis, or conditional tree forest models), timeslice, "adaptive_cv", "adaptive_boot" or "adaptive_LGOCV"
...
predictedslda2<-predict(fitslda,validationTransformed)
CM_fitslda_validation <- confusionMatrix(predictedslda2, validationTransformed$classe)
...
OutSampleAccuracy = c(CM_fitC50_validation$overall["Accuracy"]
                      ,CM_fithdda_validation$overall["Accuracy"]
